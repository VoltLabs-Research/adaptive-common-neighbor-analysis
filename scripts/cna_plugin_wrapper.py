...
def run(command: list[str]) -> None:
    ...
    print(f"[acna-plugin] {' '.join(shlex.quote(part) for part in command)}", flush=True)
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        prog='cna_plugin_wrapper.py',
        description='Wrapper portable para Adaptive/Common Neighbor Analysis.'
    )
    ...
    if unknown:
        raise WrapperError(f'Argumentos no soportados por ACNA wrapper: {unknown}')
    return args
...
def resolve_runtime_paths() -> dict[str, Path]:
    ...
    if PLUGINS_ROOT is not None:
        repo_candidates = [
            PLUGINS_ROOT / 'CommonNeighborAnalysis/build/build/Release/common-neighbor-analysis',
            PLUGINS_ROOT / 'CommonNeighborAnalysis/build-local/build/Release/common-neighbor-analysis',
            PLUGINS_ROOT / 'CommonNeighborAnalysis/build-manual/build/Release/common-neighbor-analysis'
        ]

    return {
        'cna': resolve_binary_from_env_or_candidates(
            'VOLT_CNA_BINARY',
            'binario CommonNeighborAnalysis',
            [PLUGIN_ROOT / 'bin/common-neighbor-analysis', *repo_candidates],
...
def main() -> int:
    ...
    require_file(Path(f'{args.output_base}_cna_analysis.msgpack'))
...
if __name__ == '__main__':
    try:
        ...
        print(f'[acna-plugin] error: {error}', file=sys.stderr)
        raise SystemExit(1)
